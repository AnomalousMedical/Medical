...
namespace Medical
{
    class UnifiedToothSectionEditRenderer : EditInterfaceRenderer
    {
        private UnifiedToothSection section;

        public UnifiedToothSectionEditRenderer(UnifiedToothSection section)
        {
    ...
    class UnifiedToothSection : BehaviorObject
    {
        private String name;
        ...
        public UnifiedToothSection(String name)
        {
            this.name = name;
            boundingBox = new Box3();
            Vector3[] axes = boundingBox.getAxes();
        ...
        public void createSection(Vector3[] vertices, ReshapeableRigidBody body)
        {
            createSection(vertices, body, Vector3.Zero, Quaternion.Identity, "");
        }

        public unsafe void createSection(Vector3[] vertices, ReshapeableRigidBody body, Vector3 origin, Quaternion orientation, String tempPrefix)
        {
            fixed (float* verts = &vertices[0].x)
            {
                fixed (uint* idxs = &indexBuffer[0])
                {
                    ...
                    body.createHullRegion(tempPrefix + name, decompDesc, origin, orientation);
                }
            }
        }
        ...
        {
            editInterface.Renderer = new UnifiedToothSectionEditRenderer(this);
        }
        ...
        protected UnifiedToothSection(LoadInfo info)
            : base(info)
        {
            boundingBox = new Box3();
            Vector3[] axes = boundingBox.getAxes();

        ...
        public void updateStatus(Download download)
        {
            if (UpdateStatus != null)
            {
                if (download.TotalSize > 0)
                {
                    ...
                    StatusString = String.Format("{0}\n{4} - {1}%\n{2} of {3} at {5} per second", 
                        Name, 
                        (int)((float)download.TotalRead / download.TotalSize * 100.0f),
                        Engine.Prettify.GetSizeReadable(download.TotalRead),
                        Engine.Prettify.GetSizeReadable(download.TotalSize),
                        download.StatusString, 
                        Engine.Prettify.GetSizeReadable(download.DownloadSpeed));
                }
                else
                {
